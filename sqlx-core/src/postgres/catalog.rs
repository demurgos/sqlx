...
scoped_thread_local!(static CATALOG_CONTEXT: LocalPgCatalog);
...
pub(crate) struct LocalPgCatalog {
    ...
    /// Cache of owned resolve types.
    ///
    /// Most of SQLx requires a static type information struct. This field acts
    /// as a bridge between the internal representation in the catalog and the
    /// rest of SQLx.
    resolved_types: HashMap<PgTypeCacheKey, Arc<PgType<PgTypeInfoPtr>>>,
...
/// Thread-safe reference counted handle to a `LocalPgCatalog`.
#[derive(Clone, Debug)]
pub(crate) struct LocalPgCatalogHandle(Arc<RwLock<LocalPgCatalog>>);

impl LocalPgCatalogHandle {
    pub(crate) fn new(catalog: LocalPgCatalog) -> Self {
        Self(Arc::new(RwLock::new(catalog)))
    }

    pub(crate) fn read(&self) -> RwLockReadGuard<'_, LocalPgCatalog> {
        self.0.read().expect("poisoned lock")
    }
    pub(crate) fn write(&self) -> RwLockWriteGuard<'_, LocalPgCatalog> {
...
impl LocalPgCatalog {
    ...
    pub(crate) fn insert_type(&mut self, ty: PgType<PgTypeOid>) -> Result<(), InsertTypeError> {
        let refs = [PgTypeRef::Oid(ty.oid()), PgTypeRef::Name(ty.owned_name())];
    ...
    /// Internal method to advance type resolutions
    fn advance_resolutions(&mut self, initial: PgTypeOid) {
        let mut active_resolved_types = HashMap::new();
            ...
            for (oid, mut resolution) in pending {
                let new_state: ResolutionState = match resolution.resume(&self) {
                    ...
                    GeneratorState::Complete(res) => {
                        // Precompute owned resolved type
                        self.get_or_create_owned_resolved_type(&mut active_resolved_types, oid);
    ...
    /// Internal method to precompute a `PgType<ResolvedPgTypePtr>` representation
    /// once a type is resolved.
    /// Precondition:
    /// 1. All the transitive dependencies are successfully fetched and cached
    fn get_or_create_owned_resolved_type(
        &mut self,
        active: &mut HashMap<PgTypeOid, PgTypeInfoPtr>,
        oid: PgTypeOid,
    ) -> PgTypeInfoPtr {
        if let Some(ptr) = active.get(&oid) {
            return ptr.clone();
        }
        let (ty, key) = match self.get_type_and_key(&PgTypeRef::Oid(oid)) {
            Ok((ty, key)) => (ty, key),
            Err(e) => unreachable!(
                "(BUG) Root type must be fetched before calling `compute_owned_resolved_type` {:?}: {}",
                oid, e
            )
        };
        // We can't use the entry API here due to recursion
        if let Some(ptr) = self.resolved_types.get(&key) {
            return PgTypeInfoPtr {
                inner: Arc::downgrade(ptr),
            };
        }
        // Clone the shallow type so we can move it into the closure without holding onto `&self`
        let ty = ty.clone();
        let ptr = Arc::new_cyclic(|ptr| -> PgType<PgTypeInfoPtr> {
            let ptr = PgTypeInfoPtr {
                inner: Weak::clone(ptr),
            };
            active.insert(oid, ptr);
            let kind = match ty.kind {
                PgTypeKind::Simple => PgTypeKind::Simple,
                PgTypeKind::Pseudo => PgTypeKind::Pseudo,
                PgTypeKind::Domain(t) => {
                    PgTypeKind::Domain(self.get_or_create_owned_resolved_type(active, t))
                }
                PgTypeKind::Composite(c) => PgTypeKind::composite(
                    c.fields()
                        .map(|(k, t)| {
                            (
                                k.to_string(),
                                self.get_or_create_owned_resolved_type(active, *t),
                            )
                        })
                        .collect::<Vec<_>>(),
                ),
                PgTypeKind::Array(t) => {
                    PgTypeKind::Array(self.get_or_create_owned_resolved_type(active, t))
                }
                PgTypeKind::Enum(v) => PgTypeKind::Enum(v.clone()),
                PgTypeKind::Range(t) => {
                    PgTypeKind::Range(self.get_or_create_owned_resolved_type(active, t))
                }
            };
            PgType {
                oid,
        ...
        let res = PgTypeInfoPtr {
            inner: Arc::downgrade(&ptr),
        };
        self.resolved_types.insert(key, ptr);
        res
    }
}

impl LocalPgCatalog {
    pub(crate) fn get_unfetched(&self) -> impl Iterator<Item = &PgTypeRef> {
        self.type_refs.iter().filter_map(|(ty_ref, state)| {
            if *state == ObjectRefState::Declared {
                Some(ty_ref)
            } else {
                None
            }
...
impl LocalPgCatalog {
    ...
    /// Get a shallowly-resolved type from the local registry, with the cache key
    #[allow(dead_code)]
    fn get_type_and_key(
        &self,
        ty_ref: &PgTypeRef,
    ) -> Result<(&PgType<PgTypeOid>, PgTypeCacheKey), GetPgTypeError> {
        match self.type_refs.get(ty_ref) {
            None => Err(GetPgTypeError::Undeclared),
            Some(ObjectRefState::Declared) => Err(GetPgTypeError::Unfetched),
            Some(ObjectRefState::Missing) => Err(GetPgTypeError::Missing),
            Some(ObjectRefState::Fetched(cache_key)) => {
                Ok((self.get_type_by_cache_key(cache_key), *cache_key))
            }
        }
    }
...
#[error("no fully-resolved type in the registry for the provided reference {typ:?}, try to preload the type")]
pub(crate) struct ResolvePgTypeError {
    ...
    pub(crate) typ: PgTypeRef,
    /// Reason why `dep` is preventing resolution
    pub(crate) error: GetPgTypeError,
}
...
impl LocalPgCatalog {
    ...
    /// Owned version of `resolve_type`.
    pub(crate) fn resolve_type_info(
        &self,
        ty_ref: &PgTypeRef,
    ) -> Result<PgTypeInfo, ResolvePgTypeError> {
        match self.get_type_complete(ty_ref) {
            Ok((cache_key, _ty, state)) => match state {
                ResolutionState::Full(_) => {
                    let ty = match self.resolved_types.get(&cache_key) {
                        Some(ty) => ty,
                        None => unreachable!(
                            "(BUG) No fully-resolved type in the local catalog for the key {:?}",
                            cache_key
    ...
    /// Try to get a smart reference to a deeply-resolved type from the local catalog.
    pub(crate) fn resolve_type_info_ptr(
        &self,
        oid: PgTypeOid,
    ) -> Result<PgTypeInfoPtr, ResolvePgTypeError> {
        match self.get_type_complete(&PgTypeRef::Oid(oid)) {
            Ok((cache_key, _ty, state)) => match state {
                ResolutionState::Full(_) => {
                    let inner = match self.resolved_types.get(&cache_key) {
                        Some(ty) => Arc::downgrade(ty),
                        None => unreachable!(
                            "(BUG) No fully-resolved type in the local catalog for the key {:?}",
                            cache_key
                        ),
                    };
                    Ok(PgTypeInfoPtr { inner })
                }
                ResolutionState::Partial(blocker) => {
                    return Err(ResolvePgTypeError {
                        typ: PgTypeRef::Oid(blocker),
...
impl<'catalog> ResolvedPgTypeRef<'catalog> {
    /// Get a view for the inner `ResolvedPgType`
    fn get(&self) -> ResolvedPgType<'catalog> {
        self.catalog
...
impl<'catalog> Eq for ResolvedPgTypeRef<'catalog> {}
...
impl<'catalog> Eq for ResolvedPgCompositeKind<'catalog> {}
...
/// Similar to `ResolvedPgTypeRef` but using reference-counting pointers.
#[derive(Debug, Clone)]
pub struct PgTypeInfoPtr {
    inner: Weak<PgType<PgTypeInfoPtr>>,
}

impl PartialEq for PgTypeInfoPtr {
    fn eq(&self, other: &Self) -> bool {
        std::ptr::eq(self.inner.as_ptr(), other.inner.as_ptr())
    }
}

impl Eq for PgTypeInfoPtr {}

impl PgTypeInfoPtr {
    pub fn try_get(&self) -> Option<PgTypeInfo> {
        self.inner.upgrade().map(PgTypeInfo)
    }

    pub fn get(&self) -> PgTypeInfo {
        // TODO: Add unique id to `PgLocalCatalog` and store it in `PgTypeInfoPtr`, if we also add tracing of drops/clears this should help with debugging.
        self.try_get().expect("failed to deref `PgTypeInfoPtr`: the corresponding `PgLocalCatalog` has been dropped or cleared; avoid long-lived `PgTypeInfoPtr` values to prevent this error")
    }
}
...
mod test {
    ...
    use crate::postgres::type_info2::{
        ConstFromPgBuiltinType, PgBuiltinType, PgCompositeKind, PgType, PgTypeKind, PgTypeOid,
    };
    ...
    fn test_linked_list_of_int4_by_self() {
        ...
        {
            let actual = registry.resolve_type_info(oid);
            assert_eq!(
                actual,
                Ok(PgType {
                    oid,
                    ...
                    kind: PgTypeKind::composite(vec![
                        (
                            "value".to_string(),
                            registry
                                .resolve_type_info_ptr(PgBuiltinType::Int4.oid())
                                .unwrap()
                        ),
                        (
                            "next".to_string(),
                            registry.resolve_type_info_ptr(oid).unwrap()
                        ),
                    ]),
                })
            );
